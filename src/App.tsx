...
export default function App() {
  ...
  return (
    <div className="w-screen h-screen flex flex-col overflow-hidden bg-white">
      ...
      {/* ── Header ── */}
      <header className="flex-none h-14 bg-white border-b border-gray-200 flex items-center justify-between px-4 z-40 shrink-0">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-[17px] font-semibold text-gray-800 tracking-tight">Paletta</span>
        </div>

        <div className="flex items-center gap-2">
          <button
            onClick={handleShare}
            className="flex items-center gap-1.5 px-4 h-9 rounded-full text-gray-600 hover:text-gray-900 hover:bg-gray-100 text-[13px] font-medium transition-all duration-150"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 h-9 rounded-full text-white text-[13px] font-medium transition-all duration-150 hover:opacity-90 active:scale-95"
              ...
              {helpOpen && (
                <div className="absolute bottom-10 left-0 z-50 w-52 rounded-xl bg-white border border-gray-200 shadow-xl p-4 text-[12px] text-gray-600 leading-relaxed">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="flex justify-between"><span>Undo</span><kbd className="px-1.5 py-0.5 rounded bg-gray-100 font-mono text-[11px]">⌘Z</kbd></div>
                  ...
                  <button className="absolute top-3 right-3 text-gray-300 hover:text-gray-500 text-[13px]" onClick={() => setHelpOpen(false)}>✕</button>
                </div>
              )}
            </div>
            {lockedCount > 0 && (
