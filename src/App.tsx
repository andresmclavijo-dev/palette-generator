    ...
    <div className="w-screen h-screen flex flex-col overflow-hidden bg-white">
      ...
      <header className="flex-none h-16 sm:h-14 bg-white border-b border-gray-200 flex items-center justify-between px-3 sm:px-4 z-40 shrink-0">
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          {/* Hamburger — always on mobile, desktop only when signed in */}
          <button
            onClick={() => setDrawerOpen(true)}
            className={`${isSignedIn ? '' : 'sm:hidden '}w-9 h-9 rounded-full flex items-center justify-center text-gray-600 hover:bg-gray-100 transition-all`}
            aria-label="Menu"
          >
          ...
          {/* Sign In — desktop only, hidden when signed in */}
          {!isSignedIn && (
            <button
